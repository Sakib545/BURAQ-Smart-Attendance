...
def register(staff_id,phone):
    ...
    with get_db() as c:
        e=c.execute("SELECT * FROM employees WHERE LOWER(staff_id)=LOWER(?)",(staff_id.strip(),)).fetchone()
        ...
        if normalize_phone(e['phone'])==phone:
            ...
            c.execute("DELETE FROM conversation_states WHERE phone=?",(phone,)); return f"✅ Registration সফল\nনাম: {e['name']}\nStaff ID: {e['staff_id']}",True
        c.execute("UPDATE pending_registrations SET status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE employee_id=? AND status='pending'",(e['id'],))
        ...
        c.execute("DELETE FROM conversation_states WHERE phone=?",(phone,))
    return "⏳ নম্বর মেলেনি। Admin approval-এর জন্য পাঠানো হয়েছে।",True
...
def log(direction,phone,typ,content,message_id=None):
    try:
        ...
        # Unique incoming message_id means Meta retried the same webhook.
        return False
