...
app = FastAPI(title=settings.app_name, version="9.16.0", docs_url=None, redoc_url=None)
...
.easy-flow{display:grid;gap:18px}.flow-step{display:grid;grid-template-columns:42px 1fr;gap:13px;align-items:start}.step-number{width:36px;height:36px;border-radius:12px;background:var(--brand);color:#fff;display:grid;place-items:center;font-weight:900}.step-title{font-weight:850;margin:2px 0 4px}.helper{font-size:12px;color:var(--muted);margin-top:-8px;margin-bottom:12px}.shortcut-row{display:flex;gap:8px;flex-wrap:wrap;margin:4px 0 14px}.shortcut{border:1px solid var(--line);background:var(--panel2);color:var(--ink);padding:8px 11px;border-radius:10px;font-weight:750;cursor:pointer}.shortcut:hover{border-color:var(--brand);color:var(--brand)}.form-section{padding:16px;border:1px solid var(--line);border-radius:14px;background:var(--panel2)}.form-actions{display:flex;justify-content:flex-end;gap:10px;padding-top:8px}.sticky-actions{position:sticky;bottom:12px;background:color-mix(in srgb,var(--panel) 94%,transparent);backdrop-filter:blur(8px);border:1px solid var(--line);border-radius:14px;padding:12px;display:flex;justify-content:flex-end;gap:10px;z-index:4}.advanced-box{margin-top:16px}.advanced-box summary{cursor:pointer;font-weight:800;color:var(--brand);padding:4px}.payroll-steps{display:grid;gap:14px}.field-badge{display:inline-flex;padding:4px 8px;border-radius:999px;background:#dcfce7;color:#166534;font-size:11px;font-weight:850;margin-left:7px}.manual-badge{background:#fef3c7;color:#92400e}.range-preview{padding:12px;border-radius:12px;background:var(--panel2);border:1px dashed var(--line);font-size:13px}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:14px}.table-wrap table{min-width:760px}.page-intro{display:flex;justify-content:space-between;gap:16px;align-items:flex-end;margin-bottom:16px}.page-intro h2{margin:0}.compact-card{padding:16px}.money-input{font-size:18px;font-weight:800}.safe-note{padding:12px;border-radius:12px;background:#ecfdf5;color:#166534;border:1px solid #bbf7d0}.warning-note{padding:12px;border-radius:12px;background:#fffbeb;color:#92400e;border:1px solid #fde68a}
</style>
"""
...
def layout(title: str, body: str, request: Request | None = None, active: str = ""):
    ...
    script = """<script>(function(){const root=document.documentElement;const saved=localStorage.getItem('buraq-theme');if(saved)root.dataset.theme=saved;document.getElementById('themeToggle')?.addEventListener('click',()=>{const next=root.dataset.theme==='dark'?'light':'dark';root.dataset.theme=next;localStorage.setItem('buraq-theme',next);});const iso=d=>{const x=new Date(d.getTime()-d.getTimezoneOffset()*60000);return x.toISOString().slice(0,10)};document.querySelectorAll('.easy-duty-form').forEach(form=>{const preset=form.querySelector('.duty-preset');const custom=form.querySelector('.custom-time-fields');const toggle=()=>{if(custom)custom.style.display=preset?.value==='custom'?'grid':'none'};preset?.addEventListener('change',toggle);toggle();form.querySelectorAll('.date-shortcut').forEach(btn=>btn.addEventListener('click',()=>{const now=new Date();let a=new Date(now),b=new Date(now);if(btn.dataset.range==='tomorrow'){a.setDate(a.getDate()+1);b=new Date(a)}else if(btn.dataset.range==='week'){const day=(a.getDay()+6)%7;a.setDate(a.getDate()-day);b=new Date(a);b.setDate(b.getDate()+6)}else if(btn.dataset.range==='month'){a=new Date(now.getFullYear(),now.getMonth(),1);b=new Date(now.getFullYear(),now.getMonth()+1,0)}form.querySelector('[name=start_date]').value=iso(a);form.querySelector('[name=end_date]').value=iso(b)}));form.addEventListener('submit',e=>{const a=form.querySelector('[name=start_date]')?.value,b=form.querySelector('[name=end_date]')?.value;if(a&&b&&b<a){e.preventDefault();alert('End Date cannot be earlier than Start Date.')}});});})();</script>"""
...
def startup():
    ...
    logger.info("BURAQ v9.16.0 started database=%s employees_synced=%s", database_kind(), imported)
...
@app.get("/health")
def health():
    return {"status": "ok", "service": settings.app_name, "version": "9.16.0"}
...
def employee_profile(request: Request, employee_id: int, month: str = ""):
    ...
    if can_payroll_view:
        ...
        if can_payroll_manage:
            payroll_form=f"""<div class='card'><div class='card-head'><div><h3>{'Update' if current_payroll else 'Create'} Salary</h3><div class='sub'>Attendance summary is automatic. OT, bonus and adjustments are entered by HR.</div></div><span class='tag'>Private</span></div><form method='post' action='/payroll'><input type='hidden' name='employee_id' value='{employee_id}'><input type='hidden' name='profile_employee_id' value='{employee_id}'><input type='hidden' name='overtime_mode' value='manual'><div class='form-section'><div class='step-title'>1. Salary month and basic salary</div><div class='two'><div><label>Salary Month</label><input type='month' name='salary_month' value='{escape(month)}' required></div><div><label>Basic Salary <span class='field-badge'>Master</span></label><input class='money-input' type='number' min='0' step='0.01' name='fixed_salary' value='{fixed:.2f}' required></div></div></div><div class='section-gap'></div><div class='form-section'><div class='step-title'>2. Manual overtime</div><div class='two'><div><label>OT Hours <span class='field-badge manual-badge'>Manual</span></label><input type='number' min='0' step='0.01' name='overtime_hours' value='{hours:.2f}'></div><div><label>OT Rate / Hour</label><input type='number' min='0' step='0.01' name='overtime_rate' value='{rate:.2f}'></div></div><div class='helper'>OT attendance থেকে automatic নেওয়া হবে না। HR approved hours লিখবে।</div></div><div class='section-gap'></div><div class='form-section'><div class='step-title'>3. Additions and deductions</div><div class='two'><div><label>Bonus</label><input type='number' min='0' step='0.01' name='bonus' value='{bonus:.2f}'><label>Advance</label><input type='number' min='0' step='0.01' name='advance' value='{advance:.2f}'></div><div><label>Fine</label><input type='number' min='0' step='0.01' name='fine' value='{fine:.2f}'><label>Other Deduction</label><input type='number' min='0' step='0.01' name='deduction' value='{deduction:.2f}'></div></div><label>Adjustment Reason</label><input name='adjustment_reason' value='{escape(adjustment_reason)}' placeholder='Required when bonus/deduction is used'><label>Private Note</label><textarea name='note'>{escape(current_payroll['note'] or '') if current_payroll else ''}</textarea></div><div class='form-actions'><button class='btn'>Calculate & Save Draft</button></div></form></div>"""
        history=[]
        for p in payroll:
...
def employee_duty_page(request: Request, employee_id: int, saved: str=""):
    ...
    if can_manage:
        ...
        easy_form=f"""<div class='card'><div class='card-head'><div><div class='eyebrow'>Easy Duty Assignment</div><h2>Assign Duty by Date Range</h2><div class='sub'>একদিন, কয়েকদিন বা পুরো মাস—একবারেই save করুন।</div></div><span class='tag'>Recommended</span></div><form method='post' action='/custom-duties-range' class='easy-duty-form'><input type='hidden' name='employee_id' value='{employee_id}'><div class='easy-flow'><div class='flow-step'><div class='step-number'>1</div><div><div class='step-title'>Date range</div><div class='shortcut-row'><button type='button' class='shortcut date-shortcut' data-range='today'>Today</button><button type='button' class='shortcut date-shortcut' data-range='tomorrow'>Tomorrow</button><button type='button' class='shortcut date-shortcut' data-range='week'>This Week</button><button type='button' class='shortcut date-shortcut' data-range='month'>This Month</button></div><div class='two'><div><label>Start Date</label><input type='date' name='start_date' required></div><div><label>End Date</label><input type='date' name='end_date' required></div></div></div></div><div class='flow-step'><div class='step-number'>2</div><div><div class='step-title'>Shift time</div><label>Shift Preset</label><select name='preset' class='duty-preset'><option value='morning'>Morning (08:00–16:00)</option><option value='evening'>Evening (16:00–22:00)</option><option value='night'>Night (22:00–06:00 next day)</option><option value='custom'>Custom Time</option></select><div class='two custom-time-fields' style='display:none'><div><label>Start Time</label><input type='time' name='start_time'></div><div><label>End Time</label><input type='time' name='end_time'></div></div></div></div><div class='flow-step'><div class='step-number'>3</div><div><div class='step-title'>Office and note</div><label>Office</label><input name='office_name' value='{escape(e['office_name'] or 'BURAQ Office')}'><label>Note <span class='sub'>(optional)</span></label><input name='note' placeholder='Reason or special instruction'></div></div></div><div class='range-preview'>Start Date থেকে End Date পর্যন্ত প্রতিটি তারিখে এই duty assign হবে। Existing date থাকলে update হবে।</div><div class='form-actions'><button class='btn'>Save Duty Range</button></div></form></div>"""
        advanced=f"""<details class='advanced-box card'><summary>Advanced: weekly recurring schedule</summary><div class='section-gap'></div><form method='post' action='/employees/{employee_id}/duty/regular'><label>Weekday</label><select name='weekday'>{day_options}</select><label>Shift preset</label><select name='preset'><option value='morning'>Morning (08:00-16:00)</option><option value='evening'>Evening (16:00-22:00)</option><option value='night'>Night (22:00-06:00)</option><option value='custom'>Custom</option></select><div class='two'><div><label>Custom start</label><input type='time' name='start_time'></div><div><label>Custom end</label><input type='time' name='end_time'></div></div><label>Office</label><input name='office_name' value='{escape(e['office_name'] or 'BURAQ Office')}'><button class='btn secondary'>Save Weekly Duty</button></form></details>"""
        forms=easy_form+advanced
...
def _calculate_employee_payroll(employee_id: int, month: str, fixed_salary: float, overtime_rate: float, overtime_mode: str="manual", manual_overtime_hours: float=0, bonus: float=0, advance: float=0, fine: float=0, deduction: float=0):
    ...
    # BURAQ policy: overtime is always entered and approved manually by HR.
...
def _salary_sheet_rows(month: str):
    ...
    output=[]
    for row in rows:
        item=dict(row); fixed=float(row['fixed_salary'] or 0); rate=float(row['overtime_rate'] or 0); mode='manual'
...
def payroll_page(request: Request, month: str="", saved: str="", error: str=""):
    ...
    form=""
    if can_manage:
        form=f"""<div class='card'><div class='card-head'><div><h2>Prepare One Employee</h2><div class='sub'>Month → Employee → Manual OT → Adjustments → Save Draft</div></div><span class='tag'>Simple Flow</span></div><form method='post' action='/payroll'><input type='hidden' name='return_month' value='{month}'><input type='hidden' name='overtime_mode' value='manual'><div class='payroll-steps'><div class='form-section'><div class='step-title'>1. Employee and month</div><label>Employee</label><select name='employee_id' required>{employee_options}</select><label>Salary Month</label><input type='month' name='salary_month' value='{month}' required></div><div class='form-section'><div class='step-title'>2. Salary and manual OT</div><div class='two'><div><label>Basic Salary</label><input class='money-input' type='number' min='0' step='0.01' name='fixed_salary' required></div><div><label>OT Rate / Hour</label><input type='number' min='0' step='0.01' name='overtime_rate' value='0'></div></div><label>Approved OT Hours <span class='field-badge manual-badge'>Manual only</span></label><input type='number' min='0' step='0.01' name='overtime_hours' value='0'><div class='helper'>Late checkout বা attendance থেকে OT automatic হবে না।</div></div><div class='form-section'><div class='step-title'>3. Adjustments</div><div class='two'><div><label>Bonus</label><input type='number' min='0' step='0.01' name='bonus' value='0'><label>Salary Advance</label><input type='number' min='0' step='0.01' name='advance' value='0'></div><div><label>Fine</label><input type='number' min='0' step='0.01' name='fine' value='0'><label>Other Deduction</label><input type='number' min='0' step='0.01' name='deduction' value='0'></div></div><label>Adjustment Reason</label><input name='adjustment_reason' placeholder='Required for bonus, advance, fine or deduction'><label>Private HR Note</label><textarea name='note'></textarea></div></div><div class='form-actions'><button class='btn'>Calculate & Save Draft</button></div></form></div>"""
    table=[]
    for r in rows:
        controls=""
...
def duty_schedules_page(request: Request, saved: str=""):
    ...
    if can_manage:
        day_options=''.join(f"<option value='{i}'>{d}</option>" for i,d in enumerate(days))
        form=f"""<div class='card'><div class='card-head'><div><div class='eyebrow'>Easy Duty Assignment</div><h2>Assign Date Range</h2><div class='sub'>Employee, dates and shift নির্বাচন করে একবারেই duty save করুন।</div></div><span class='tag'>Fast</span></div><form method='post' action='/custom-duties-range' class='easy-duty-form'><div class='easy-flow'><div class='flow-step'><div class='step-number'>1</div><div><div class='step-title'>Employee</div><select name='employee_id' required>{options}</select></div></div><div class='flow-step'><div class='step-number'>2</div><div><div class='step-title'>Start and end date</div><div class='shortcut-row'><button type='button' class='shortcut date-shortcut' data-range='today'>Today</button><button type='button' class='shortcut date-shortcut' data-range='tomorrow'>Tomorrow</button><button type='button' class='shortcut date-shortcut' data-range='week'>This Week</button><button type='button' class='shortcut date-shortcut' data-range='month'>This Month</button></div><div class='two'><div><label>Start Date</label><input type='date' name='start_date' required></div><div><label>End Date</label><input type='date' name='end_date' required></div></div></div></div><div class='flow-step'><div class='step-number'>3</div><div><div class='step-title'>Shift</div><select name='preset' class='duty-preset'><option value='morning'>Morning (08:00–16:00)</option><option value='evening'>Evening (16:00–22:00)</option><option value='night'>Night (22:00–06:00 next day)</option><option value='custom'>Custom Time</option></select><div class='two custom-time-fields' style='display:none'><div><label>Start Time</label><input type='time' name='start_time'></div><div><label>End Time</label><input type='time' name='end_time'></div></div><label>Office</label><input name='office_name' value='BURAQ Office'><label>Note <span class='sub'>(optional)</span></label><input name='note' placeholder='Special instruction'></div></div></div><div class='range-preview'>Selected range-এর প্রতিটি date-এ duty তৈরি হবে। একই date থাকলে update হবে।</div><div class='form-actions'><button class='btn'>Save Duty Range</button></div></form></div>"""
        custom_form=f"""<details class='advanced-box card'><summary>Advanced: weekly recurring roster</summary><div class='section-gap'></div><form method='post' action='/duty-schedules'><label>Employee</label><select name='employee_id'>{options}</select><label>Weekday</label><select name='weekday'>{day_options}</select><div class='two'><div><label>Start</label><input type='time' name='start_time' required></div><div><label>End</label><input type='time' name='end_time' required></div></div><label>Office</label><input name='office_name' value='BURAQ Office'><button class='btn secondary'>Save Weekly Duty</button></form></details>"""
    roster=[]
    for r in rows:
...
@app.post("/custom-duties-range")
def save_custom_duty_range(request: Request, employee_id: int=Form(...), start_date: str=Form(...), end_date: str=Form(...), preset: str=Form("morning"), start_time: str=Form(""), end_time: str=Form(""), office_name: str=Form("BURAQ Office"), note: str=Form("")):
    require_permission(request,"duty_manage")
    try:
        first=datetime.strptime(start_date,'%Y-%m-%d').date()
        last=datetime.strptime(end_date,'%Y-%m-%d').date()
    except ValueError:
        raise HTTPException(400,'Invalid duty date range')
    if last < first:
        raise HTTPException(400,'End Date cannot be earlier than Start Date')
    if (last-first).days > 366:
        raise HTTPException(400,'Duty range cannot exceed 366 days')
    ...
    with get_db() as c:
        ...
        while day<=last:
            ...
            c.execute("INSERT INTO custom_duties(employee_id,duty_date,start_time,end_time,office_name,note,created_by) VALUES(?,?,?,?,?,?,?) ON CONFLICT(employee_id,duty_date) DO UPDATE SET start_time=excluded.start_time,end_time=excluded.end_time,office_name=excluded.office_name,note=excluded.note,is_active=excluded.is_active,updated_at=CURRENT_TIMESTAMP",(employee_id,date_text,start_time,end_time,office_name.strip() or 'BURAQ Office',note.strip() or None,actor))
            c.execute("DELETE FROM duty_reminder_logs WHERE employee_id=? AND duty_date=?",(employee_id,date_text))
            count+=1
            day+=timedelta(days=1)
        audit(request,'save','custom_duty_range',f'{employee_id}:{start_date}:{end_date}',f'{count} days, {start_time}-{end_time}',db=c)
    return RedirectResponse('/duty-schedules?saved=1',303)
