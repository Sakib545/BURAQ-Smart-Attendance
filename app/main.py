...
a{color:inherit}.shell{min-height:100vh;display:grid;grid-template-columns:272px 1fr}.sidebar{background:linear-gradient(180deg,#0b3328 0%,#0a2c23 100%);color:#fff;padding:22px 16px;position:sticky;top:0;height:100vh;border-right:1px solid rgba(255,255,255,.06)}.brand-block{padding:4px 8px 20px}.logo{font-size:21px;font-weight:900;line-height:1.2;margin-bottom:7px;letter-spacing:-.3px}.logo:before{content:'◆';color:#69ddb8;margin-right:9px;font-size:15px}.side-sub{font-size:12px;color:#b8d4ca;margin-bottom:24px;line-height:1.45}.side-label{padding:0 12px 8px;font-size:10px;font-weight:850;letter-spacing:1.3px;text-transform:uppercase;color:#7fae9e}.side-nav{display:grid;gap:6px}.side-nav a{padding:11px 13px;border-radius:12px;text-decoration:none;color:#d7ebe4;font-weight:700;display:flex;align-items:center;gap:10px;transition:.16s ease}.side-nav a:hover{background:rgba(255,255,255,.08);color:#fff}.side-nav a.active{background:#fff;color:#0d3b2e;box-shadow:0 8px 22px rgba(0,0,0,.14)}.side-nav a.logout{margin-top:8px;color:#ffd6d6}.main{min-width:0}.topbar{min-height:78px;background:color-mix(in srgb,var(--panel) 94%,transparent);backdrop-filter:blur(12px);border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;padding:13px 30px;position:sticky;top:0;z-index:5}.page{padding:28px 30px 40px;max-width:1480px;margin:auto}.title{font-size:25px;font-weight:900;letter-spacing:-.55px}.sub{color:var(--muted);font-size:13px;line-height:1.5}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.two{display:grid;grid-template-columns:1fr 1fr;gap:18px}.card{background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:20px;box-shadow:0 8px 26px rgba(22,59,49,.055)}.metric{font-size:31px;font-weight:900;margin-top:7px;letter-spacing:-1px}.status{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:12px;font-weight:800}.status:before{content:'●';font-size:9px}.ok{background:#dcfce7;color:var(--ok)}.warn{background:#fef3c7;color:var(--warn)}.bad{background:#fee2e2;color:var(--bad)}.actions{display:flex;gap:9px;flex-wrap:wrap;align-items:center}.btn{border:1px solid transparent;border-radius:11px;padding:10px 14px;background:var(--brand);color:#fff;font-weight:800;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:7px;transition:.16s ease;min-height:40px}.btn:hover{background:var(--brand2);transform:translateY(-1px)}.btn.secondary{background:var(--panel);border-color:var(--line);color:var(--ink)}.btn.secondary:hover{border-color:var(--brand);color:var(--brand);background:var(--panel2)}.btn.danger{background:#fff1f2;color:var(--bad);border-color:#fecdd3}.btn.danger:hover{background:#ffe4e6}.btn:disabled{opacity:.55;cursor:not-allowed;transform:none}input,select,textarea{width:100%;padding:11px 12px;border:1px solid var(--line);border-radius:11px;margin:6px 0 14px;background:var(--panel);color:var(--ink);outline:none;transition:.15s ease}input:focus,select:focus,textarea:focus{border-color:var(--brand);box-shadow:0 0 0 3px rgba(8,127,91,.11)}label{font-size:13px;font-weight:800}table{width:100%;border-collapse:separate;border-spacing:0;font-size:13px}th,td{text-align:left;padding:12px;border-bottom:1px solid var(--line);vertical-align:middle}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.65px;background:var(--panel2);position:sticky;top:0}tbody tr:hover td{background:color-mix(in srgb,var(--panel2) 65%,transparent)}h2{margin:0 0 14px;font-size:19px;letter-spacing:-.2px}h3{margin:0 0 10px}.notice{padding:13px 15px;border-radius:12px;background:#ecfeff;color:#155e75;margin-bottom:16px;border:1px solid #a5f3fc}.code{font-family:ui-monospace,monospace;background:#111827;color:#f9fafb;padding:12px;border-radius:10px;overflow:auto}.login{max-width:460px;margin:7vh auto;padding:18px}.login .card{padding:32px}.masked{font-family:ui-monospace,monospace;letter-spacing:.5px;background:var(--panel2);padding:11px;border-radius:10px;border:1px solid var(--line)}.section-gap{height:18px}.mobile-menu{display:none}.health-list{display:grid;gap:10px}.health-row{display:flex;justify-content:space-between;align-items:center;padding:11px 0;border-bottom:1px solid var(--line)}.page-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}.page-head .sub{max-width:720px}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:14px}.muted-box{background:var(--panel2);border:1px solid var(--line);border-radius:13px;padding:14px}.empty-state{text-align:center;padding:34px 18px;color:var(--muted)}
...
.sidebar{display:flex;flex-direction:column;overflow-y:auto}.side-nav{flex:0 0 auto}.side-account{margin-top:auto;padding:13px;border-radius:14px;background:rgba(255,255,255,.08);flex:0 0 auto;border:1px solid rgba(255,255,255,.07)}.side-account .side-sub{margin:3px 0 0}.mobile-panel{position:absolute;right:16px;top:62px;min-width:230px;background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:8px;box-shadow:var(--shadow);display:grid;z-index:20}.mobile-panel a{padding:11px;text-decoration:none;border-radius:9px}.mobile-panel a.active{background:var(--panel2);color:var(--brand);font-weight:800}.mobile-menu summary{list-style:none}.top-user{display:flex;align-items:center;gap:10px}.top-avatar{width:38px;height:38px;border-radius:12px;background:var(--panel2);border:1px solid var(--line);display:grid;place-items:center;font-weight:900;color:var(--brand)}
...
@media(max-width:900px){.summary-strip{grid-template-columns:1fr 1fr}.shell{grid-template-columns:1fr}.sidebar{display:none}.grid{grid-template-columns:1fr 1fr}.two{grid-template-columns:1fr}.mobile-menu{display:block}.page{padding:18px}.topbar{padding:12px 18px}.page-head{align-items:stretch}.top-user{display:none}}
@media(max-width:700px){.control-grid{grid-template-columns:1fr}.profile-hero{grid-template-columns:1fr}.facts{grid-template-columns:1fr 1fr}.salary-breakdown{grid-template-columns:1fr 1fr}.searchbar{grid-template-columns:1fr}.calendar{gap:4px}.cal-day{min-height:58px;padding:5px}.page-head{display:grid}.page-head .actions{width:100%}.page-head .btn{flex:1}.card{padding:16px}}
@media(max-width:540px){.grid{grid-template-columns:1fr}.topbar{height:auto;min-height:68px;padding:11px 14px;gap:10px}.title{font-size:21px}.page{padding:14px}.summary-strip{grid-template-columns:1fr 1fr}.facts{grid-template-columns:1fr}.salary-breakdown{grid-template-columns:1fr}.btn{width:auto}.actions{gap:7px}}
</style>
"""
...
def layout(title: str, body: str, request: Request | None = None, active: str = ""):
    if request is not None and logged_in(request):
        ...
        links = "".join(f"<a class='{"active" if group==k else ""}' href='{u}'><span>{icon}</span><span>{label}</span></a>" for k,icon,label,u,visible in nav if visible)
        user_name = escape(str(request.session.get("user_name", "Admin")))
        role_label = escape(role.replace("_", " ").title())
        initials = escape("".join(part[:1] for part in user_name.split()[:2]).upper() or "A")
        body = f"<div class='shell'><aside class='sidebar'><div class='brand-block'><div class='logo'>BURAQ</div><div class='side-sub'>Smart Attendance & HR</div></div><div class='side-label'>Workspace</div><nav class='side-nav'>{links}<a class='logout' href='/logout'><span>↪</span><span>Logout</span></a></nav><div class='side-account'><b>{user_name}</b><div class='side-sub'>{role_label}</div></div></aside><main class='main'><header class='topbar'><div><div class='title'>{escape(title)}</div><div class='sub'>Simple, clear and easy to manage</div></div><div class='actions'><details class='mobile-menu'><summary class='btn secondary'>☰ Menu</summary><div class='mobile-panel'>{links}<a href='/logout'>Logout</a></div></details><button id='themeToggle' class='btn secondary' type='button'>◐ Theme</button><div class='top-user'><div class='top-avatar'>{initials}</div><div><b>{user_name}</b><div class='sub'>{role_label}</div></div></div></div></header><div class='page'>{body}</div></main></div>"
