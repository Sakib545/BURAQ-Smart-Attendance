...
def extract_embedding(image_bytes: bytes, required_pose: str | None = None):
    ...
    try:
        ...
        # Anti-spoofing is an extra signal, not a dependency. If it fails on an
        # unusual frame the check-in still goes through, marked unknown.
        logger.exception("liveness analysis failed")
        spoof = liveness.LivenessResult(0.0, "unknown", {}, False)
