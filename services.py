...
def receive_image(phone, media_id, image_bytes=None):
    ...
    with get_db() as c:
        ...
        # Meta redelivers a webhook whenever it is unsure the first one landed, so
        # the same media_id can arrive twice. The first pass has already stored a
        # fingerprint row; without this filter the second pass compares the selfie
        # against itself and reports a 100% duplicate.
        exact = c.execute("""SELECT id,phash,ahash,dhash,embedding,pose,yaw,landmarks FROM attendance_fingerprints
            WHERE (phash=? OR ahash=? OR dhash=?) AND (media_id IS NULL OR media_id<>?)
            ORDER BY id DESC LIMIT 100""",(fingerprint["phash"],fingerprint["ahash"],fingerprint["dhash"],media_id)).fetchall()
        recent = c.execute("""SELECT id,phash,ahash,dhash,embedding,pose,yaw,landmarks FROM attendance_fingerprints
            WHERE employee_id=? AND (media_id IS NULL OR media_id<>?)
            ORDER BY id DESC LIMIT 120""", (employee["id"], media_id)).fetchall()
        seen=set(); prior=[]
