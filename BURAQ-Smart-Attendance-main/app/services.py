...
FIRST_SHIFT_ASSIGNMENTS = {"first", "morning", "day"}
...
def resolve_attendance_shift(employee, moment):
    ...
    * A second-shift worker who reaches the office at 15:56 for a 16:00 duty
      sits *before* a 16:00 cutoff and, on the clock alone, would be filed as
      first shift and measured against the 08:30 start — over seven hours
      "late" for arriving four minutes early.
    * A first-shift worker who arrives very late, at 16:20 for an 08:30 duty,
      sits *after* the cutoff and, on the clock alone, would be filed as second
      shift and measured against the 16:00 start — recorded as 20 minutes late
      instead of the real ~8 hours.

    So the employee's own roster assignment wins whenever it is explicit —
    first *or* second. Only when the shift column says nothing useful do we
    fall back to guessing from the clock. Per-day exceptions (an employee
    covering the other shift) are handled by a custom/weekly duty row, which
    ``duty_window`` already honours ahead of the global rule.
    """
    assigned = str((employee["shift"] if employee is not None else "") or "").strip().lower()
    if assigned in SECOND_SHIFT_ASSIGNMENTS:
        return "second"
    if assigned in FIRST_SHIFT_ASSIGNMENTS:
        return "first"
