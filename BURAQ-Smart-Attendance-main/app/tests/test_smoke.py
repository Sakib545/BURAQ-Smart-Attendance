...
def test_shift_is_automatic_and_checkout_requires_checkin():
    ...
    with TestClient(app):
        ...
        approved = approve_pending_attendance(int(fingerprint["id"]), "test-admin")
        # 'morning' is an explicit first-shift assignment, so a 16:02 check-in
        # is filed as First Shift (very late) rather than being re-labelled
        # Second Shift by the 16:00 cutoff.
        assert approved and "First Shift" in approved["result"]
        with get_db() as db:
