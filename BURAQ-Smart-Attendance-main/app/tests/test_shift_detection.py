...
def test_morning_staff_arriving_after_cutoff_stay_first():
    """A first-shift worker who arrives very late must not be re-labelled as
    second shift by the clock — that used to hide ~8 hours of lateness as ~20
    minutes. An explicit 'morning' assignment now wins over the cutoff."""
    employee = _employee(MORNING_STAFF)
    assert resolve_attendance_shift(employee, _moment("16:30")) == "first"
    # Measured against the 08:30 start: 16:30 - 08:30 = 480 minutes late.
    assert _late_for(MORNING_STAFF, "16:30") == 480


def test_truly_unassigned_staff_fall_back_to_the_clock():
    """Only when the shift column says nothing useful does the clock decide."""
    unassigned = {"shift": ""}
    assert resolve_attendance_shift(unassigned, _moment("16:30")) == "second"
    assert resolve_attendance_shift(unassigned, _moment("09:30")) == "first"
