...
def main() -> int:
    ...
    with get_db() as c:
        ...
        for row in rows:
            ...
            correct_shift = resolve_attendance_shift(employee, check_in)
            start_dt, end_dt = duty_window(
                employee, work_date, db=c,
                shift_override=_employee_shift_for_attendance(correct_shift))
